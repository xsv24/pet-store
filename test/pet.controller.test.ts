...
describe('PetController (e2e)', () => {
  ...
  it('/pets (POST)', async () => {
    // Arrange
    const req = { name: 'Joe', type: 'dog', dob: '', species: ''};
  ...
  describe('/pets (GET)', () => {
    it('existing pets are fetched', async () => {
      // Arrange
      const dog = await PostPet(server, { name: 'Joe', type: 'dog', dob: '', species: ''});
  ...
  describe('/pets/:id (delete)', () => {
    ...
    it('On pet not found a 404 is returned', async () => {
      // Arrange, Act & Assert
      await server
        .delete(`/v1/pets/${randomUUID()}`)
        .expect(404)
    });
  });
});

export async function PostPet(server: supertest.SuperTest<supertest.Test>, body?: any) : Promise<PetEntity> {
    const response = await server
      .post('/v1/pets')
      .send(body ?? { name: 'Joe', type: 'dog', dob: '', species: ''})
      .expect(201);
